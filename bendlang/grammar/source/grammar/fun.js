...
module.exports = {
  ...
  fun_function_definition: $ => choice(
    seq(
      $.def_modifier,
      $._function_patterns,
  ...
  // A signature-only declaration is used by the functional surface to
  // annotate a group of pattern rules that follow it.
  fun_function_signature: $ => seq(
    optional($.def_modifier),
    $._function_patterns,
    ':',
    field('return_type', $.type_expr),
  ),

  def_modifier: $ => choice('unchecked', 'checked', 'const'),
  ...
  _function_patterns: $ => choice(
    prec.dynamic(10, seq(
      ...
      $._function_pattern,
      ')',
      repeat(seq('(', $._function_pattern, ')')),
    )),
    $._function_pattern
  ),

  _function_pattern: $ => prec.right(seq(
  ...
  _type_constructor_field: $ => choice(
    seq('~', '(', $.identifier, optional(seq(':', $.type_expr)), ')'),
    seq('~', $.identifier, optional(seq(':', $.type_expr))),
    seq('(', $.identifier, optional(seq(':', $.type_expr)), ')'),
    seq($.identifier, optional(seq(':', $.type_expr))),
  ),
  ...
  // A let continuation must not consume another let as its final term. The
  // explicit chain rule below uses this non-recursive view to make adjacent
  // bindings deterministic for the pure-Go GLR runtime.
  _terms_no_let: $ => choice(
    $._literals,
    alias($._fun_list, $.list),
    alias($._fun_tree_node, $.tree_node),
    alias($._fun_tree_leaf, $.tree_leaf),
  ...
  _fun_args: $ => seq(
    $._fun_arg,
    repeat(seq(optional(','), $._fun_arg)),
    optional(',')
  ),

  _fun_args_id: $ => seq(
    $._fun_arg_id,
    repeat(seq(optional(','), $._fun_arg_id)),
    optional(',')
  ),
  ...
  // Keep the first term of a let chain from greedily consuming the next
  // unparenthesized `let`. The compiler treats that token as the binding
  // continuation; excluding the recursive form here makes the chain
  // deterministic for both the C and pure-Go runtimes.
  let_value: $ => choice(
    $._literals,
    alias($._fun_list, $.list),
    alias($._fun_tree_node, $.tree_node),
  ...
  // Functional lets may finish at a closing application delimiter or EOF;
  // left associativity keeps the continuation attached to the innermost let
  // while preserving the compiler's line-oriented newline when present.
  let_next: $ => prec.left(seq(optional(SEMICOLON), $._terms_no_let, optional($._newline))),
  ...
  fun_local_def: $ => seq(
    ...
    alias($._terms_no_let, $.nxt)
  ),

  operator: $ => prec(PREC.times, choice(
    '+',
    '-',
    '**',
    '*',
